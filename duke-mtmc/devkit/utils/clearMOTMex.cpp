	...
	int nrhs, const mxArray *prhs[])
{
	...
	vector<vector<bool>> gtInd(Fgt, vector<bool>(Ngt, false));
	vector<vector<bool>> stInd(F, vector<bool>(N, false));
	for (int i = 0; i < Fgt; i++) {
		for (int j = 0; j < Ngt; j++) {
			gtInd[i][j] = !(!gtXi[index(i, j, Fgt )]);
		}
	}
	for (int i = 0; i < F; i++) {
		for (int j = 0; j < N; j++) {
			stInd[i][j] = !(!stXi[index(i, j, F )]);
		}
	}
	double INF = 1e9; 
	vector<vector<int>> M(F, vector<int>(Ngt, -1));  // mapping
	...
	for (int t = 0; t < F; t++) {
		for (int k = 0; k < Ngt; k++)  if (gtInd[t][k]) g[t]++;
	}
	...
	for (int t = 0; t < F; t++)
	{
		...
		{
			vector<int> mappings;
			for (int k = 0; k < Ngt; k++) if (M[t - 1][k] != -1) mappings.push_back(k);
			for (int k = 0; k < mappings.size(); k++)
			{
				if (gtInd[t][mappings[k]] && stInd[t][M[t - 1][mappings[k]]])
				{
					bool matched = false;
					if (world)
					{
		...
		for (int k = 0; k < Ngt; k++) if (M[t][k]==-1 && gtInd[t][k]) unmappedGt.push_back(k);
		for (int k = 0; k < N; k++) if (stInd[t][k]) stindt.push_back(k);
		for (int k = 0; k < Ngt; k++) if (M[t][k] != -1) findm.push_back(M[t][k]);
		...
		vector<vector<double>> alldist(max(Ngt,N), vector<double>(max(Ngt,N), INF));
		...
		for (int k = 0; k < Ngt; k++) if (M[t][k] != -1) { curtracked.push_back(k);  set1.push_back(M[t][k]); }
		for (int k = 0; k < N; k++) if (stInd[t][k]) alltrackers.push_back(k);
		...
		for (int k = 0; k < Ngt; k++) alltracked[t][k] = M[t][k];
		...
		if (t > 0)
		{
			for (int k = 0; k < curtracked.size(); k++)
			{
				...
				if (gtInd[t - 1][ct] && lastnonempty != -1)
				{
					if (M[t][ct] != M[lastnonempty][ct]) 
						mme[t]++;
				}
			}
		}
		...
		for (int k = 0; k < N; k++) if (stInd[t][k]) fp[t]++;
		fp[t] -= c[t];
		m[t] = g[t] - c[t];
	}

	for (int k = 0; k < F; k++) mmeOut[k] = mme[k];
	for (int k = 0; k < F; k++) cOut[k] = c[k];
	for (int k = 0; k < F; k++) fpOut[k] = fp[k];
	for (int k = 0; k < F; k++) gOut[k] = g[k];
	for (int k = 0; k < F; k++) mOut[k] = m[k];
